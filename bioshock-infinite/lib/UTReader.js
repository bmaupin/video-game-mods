module.exports = function (arrayBuffer) {
	...
	this.getCompactIndex = function (startValue) {
		...
		for (let i = 1; i < Math.min(length, 4); i++) {
			value |=
				(reader.dataView.getUint8(reader.offset + i) & 0x7f) <<
				(6 + (i - 1) * 7);
		}
	...
	class BspNode {
		constructor() {
			...
			this.i_zone = [reader.getCompactIndex(), reader.getCompactIndex()];

			this.vertices = reader.getUint8();

			this.i_leaf = [reader.getUint32(), reader.getUint32()];
		}
	}
	...
	this.meshClasses = ['Mesh', 'LodMesh', 'SkeletalMesh'];
	...
	this.getPackageHeader = function () {
		...
		if (header.signature !== SIGNATURE_UT) {
			throw `Invalid package signature: 0x${header.signature
				.toString(16)
				.padStart(8, 0)}`;
		}
		...
		if (header.version >= 249) {
			header.size = reader.getUint32();
		}
		...
		if (header.version >= 269) {
			const textSize = reader.getUint32();
			header.folder_name = reader.getUnsizedText(reader.offset, textSize);
			console.log('header.folder_name=', header.folder_name);
		}
	...
	this.getObjectsByType = function (objectType) {
		...
		for (const exportObject of reader.exportTable) {
			...
			if (
				classObject !== null &&
				objectType ===
					reader.nameTable[classObject.object.object_name_index].name
			) {
	...
	this.getBrushData = function (brushObject) {
		...
		// Model
		const modelIndex = data.brush.properties.filter(
			(p) => p.name.toLowerCase() === 'brush'
		)[0];
	...
	this.getTextureInfo = function (textureObject) {
		...
		return {
			...
			group: objectInfo
				? reader.nameTable[objectInfo.object.object_name_index].name
				: null,
		};
	};
	...
	this.getScreenshot = function (callback) {
		...
		const screenshotNames = reader.nameTable.filter((name) =>
			screenshotRegEx.test(name)
		);
		let screenshotObjects = screenshotNames.map((name) =>
			reader.getObjectByName(name)
		);
			...
			for (let i = 0; i < screenshotObjects.length; i++) {
				reader.textureToCanvas(screenshotObjects[i], function (screenshot) {
					...
					if (screenshots.length === screenshotObjects.length) {
						...
						screenshots.sort(
							(a, b) =>
								Number(a.name.substring('Screenshot'.length)) -
								Number(b.name.substring('Screenshot'.length))
						);
				...
				for (const prop of levelInfoProps) {
					if (prop.name === 'Screenshot') {
						...
						if (invalidScreenshot.table !== 'import') {
							reader.textureToCanvas(
								invalidScreenshot.object,
								function (screenshot) {
	...
	this.getLevelSummary = function (allProperties = false) {
		...
		if (levelInfoObject) {
			...
			for (const prop of properties) {
				if (allProperties || meaningfulProperties.includes(prop.name)) {
					if (tableLookup.includes(prop.name)) {
						levelSummary[prop.name] =
							reader.nameTable[
								reader.getObject(prop.value).object.object_name_index
							].name;
					} else {
			...
			levelSummary['Screenshot'] =
				Object.keys(levelSummary).includes('Screenshot');
		}
	...
	this.getDependencies = function () {
		...
		for (const entry of reader.importTable) {
			if (
				reader.nameTable[entry.class_name_index].name === 'Package' &&
				entry.package_index === 0
			) {
				const dependency = {
	...
	this.getClassesCount = function () {
		...
		for (const object of reader.exportTable) {
			const name = reader
				.getObjectNameFromIndex(object.class_index)
				.toLowerCase();
